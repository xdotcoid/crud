...
@section('content')
    <div class="container">
        <h1>Alamat Kirim</h1>
        <form method="post" action="{{ url('konsumen/alamatkirim/store') }}" style="display:inline">
            @csrf
            <label for="nama_penerima" class="col-form-label">Nama</label>
            <input type="text" class="form-control" name="nama_penerima" value="{{ old('nama_penerima') }}" />
